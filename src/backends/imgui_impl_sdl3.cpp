...
{
    return ImGui::GetCurrentContext() ? (ImGuiSDL3Data*)ImGui::GetIO().BackendPlatformUserData : nullptr;
}

// Forward Declarations
static void ImGuiSDL3UpdateMonitors();
static void ImGuiSDL3InitMultiViewportSupport(SDL_Window* window, void* sdlGlContext);
...
static const char* ImGuiSDL3GetClipboardText(ImGuiContext*)
{
    ...
    bd->clipboardTextData = sdlClipboardText ? SDL_strdup(sdlClipboardText) : nullptr;
...
static void ImGuiSDL3PlatformSetImeData(ImGuiContext*, ImGuiViewport* viewport, ImGuiPlatformImeData* data)
{
    ImGuiSDL3Data* bd = ImGuiSDL3GetBackendData();
...
bool ImGuiSDL3ProcessEvent(const SDL_Event* event)
{
    ...
    switch (event->type)
    {
        ...
        case SDL_EVENT_MOUSE_BUTTON_UP:
        {
            ...
            bd->mouseButtonsDown = (event->type == SDL_EVENT_MOUSE_BUTTON_DOWN) ? (bd->mouseButtonsDown | (1 << mouseButton)) : (bd->mouseButtonsDown & ~(1 << mouseButton));
            return true;
        }
        ...
        case SDL_EVENT_KEY_UP:
        {
            ...
            ImGuiKey key = ImGuiSDL3KeyEventToImGuiKey(event->key.key, event->key.scancode);
        ...
        {
            ImGuiViewport* viewport = ImGuiSDL3GetViewportForWindowID(event->window.windowID);
            if (viewport == NULL)
                return false;
...
static void ImGuiSDL3SetupPlatformHandles(ImGuiViewport* viewport, SDL_Window* window)
{
...
static bool ImGuiSDL3Init(SDL_Window* window, SDL_Renderer* renderer, void* sdlGlContext)
{
    ImGuiIO& io = ImGui::GetIO();
    IMGUI_CHECKVERSION();
...
static void ImGuiSDL3UpdateMouseData()
{
    ...
    const bool isAppFocused = (focusedWindow && (bd->window == focusedWindow || ImGuiSDL3GetViewportForWindowID(SDL_GetWindowID(focusedWindow)) != NULL));
#else
    ...
    if (io.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport)
    {
        ...
        if (ImGuiViewport* mouseViewport = ImGuiSDL3GetViewportForWindowID(bd->mouseWindowId))
...
static void ImGuiSDL3UpdateMouseCursor()
{
    ...
    else
    {
        ...
        SDL_Cursor* expectedCursor = bd->mouseCursors[imguiCursor] ? bd->mouseCursors[imguiCursor] : bd->mouseCursors[ImGuiMouseCursor_Arrow];
...
void ImGuiSDL3SetGamepadMode(ImGuiSDL3GamepadMode mode, SDL_Gamepad** manualGamepadsArray, int manualGamepadsCount)
{
    ImGuiSDL3Data* bd = ImGuiSDL3GetBackendData();
    ImGuiSDL3CloseGamepads();
...
static void ImGuiSDL3UpdateGamepadButton(ImGuiSDL3Data* bd, ImGuiIO& io, ImGuiKey key, SDL_GamepadButton buttonNo)
{
...
static void ImGuiSDL3UpdateGamepadAnalog(ImGuiSDL3Data* bd, ImGuiIO& io, ImGuiKey key, SDL_GamepadAxis axisNo, float v0, float v1)
{
    float mergedValue = 0.0f;
    for (SDL_Gamepad* gamepad : bd->gamepads)
    {
        float vn = Saturate((float)(SDL_GetGamepadAxis(gamepad, axisNo) - v0) / (float)(v1 - v0));
        if (mergedValue < vn)
            mergedValue = vn;
    }
...
static void ImGuiSDL3UpdateGamepads()
{
    ...
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadStart,       SDL_GAMEPAD_BUTTON_START);
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadBack,        SDL_GAMEPAD_BUTTON_BACK);
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadFaceLeft,    SDL_GAMEPAD_BUTTON_WEST);           // Xbox X, PS Square
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadFaceRight,   SDL_GAMEPAD_BUTTON_EAST);           // Xbox B, PS Circle
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadFaceUp,      SDL_GAMEPAD_BUTTON_NORTH);          // Xbox Y, PS Triangle
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadFaceDown,    SDL_GAMEPAD_BUTTON_SOUTH);          // Xbox A, PS Cross
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadDpadLeft,    SDL_GAMEPAD_BUTTON_DPAD_LEFT);
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadDpadRight,   SDL_GAMEPAD_BUTTON_DPAD_RIGHT);
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadDpadUp,      SDL_GAMEPAD_BUTTON_DPAD_UP);
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadDpadDown,    SDL_GAMEPAD_BUTTON_DPAD_DOWN);
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadL1,          SDL_GAMEPAD_BUTTON_LEFT_SHOULDER);
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadR1,          SDL_GAMEPAD_BUTTON_RIGHT_SHOULDER);
    ImGuiSDL3UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadL2,          SDL_GAMEPAD_AXIS_LEFT_TRIGGER,  0.0f, 32767);
    ImGuiSDL3UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadR2,          SDL_GAMEPAD_AXIS_RIGHT_TRIGGER, 0.0f, 32767);
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadL3,          SDL_GAMEPAD_BUTTON_LEFT_STICK);
    ImGuiSDL3UpdateGamepadButton(bd, io, ImGuiKey_GamepadR3,          SDL_GAMEPAD_BUTTON_RIGHT_STICK);
    ImGuiSDL3UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickLeft,  SDL_GAMEPAD_AXIS_LEFTX,  -thumbDeadZone, -32768);
    ImGuiSDL3UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickRight, SDL_GAMEPAD_AXIS_LEFTX,  +thumbDeadZone, +32767);
    ImGuiSDL3UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickUp,    SDL_GAMEPAD_AXIS_LEFTY,  -thumbDeadZone, -32768);
    ImGuiSDL3UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickDown,  SDL_GAMEPAD_AXIS_LEFTY,  +thumbDeadZone, +32767);
    ImGuiSDL3UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickLeft,  SDL_GAMEPAD_AXIS_RIGHTX, -thumbDeadZone, -32768);
    ImGuiSDL3UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickRight, SDL_GAMEPAD_AXIS_RIGHTX, +thumbDeadZone, +32767);
    ImGuiSDL3UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickUp,    SDL_GAMEPAD_AXIS_RIGHTY, -thumbDeadZone, -32768);
    ImGuiSDL3UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickDown,  SDL_GAMEPAD_AXIS_RIGHTY, +thumbDeadZone, +32767);
}
...
struct ImGuiSDL3ViewportData
{
    ...
    ImGuiSDL3ViewportData() { window = parentWindow = nullptr; windowId = 0; windowOwned = false; glContext = nullptr; }
    ~ImGuiSDL3ViewportData() { IM_ASSERT(window == nullptr && glContext == nullptr); }
};
...
static void ImGuiSDL3CreateWindow(ImGuiViewport* viewport)
{
    ...
    vd->parentWindow = ImGuiSDL3GetSDLWindowFromViewportID(viewport->ParentViewportId);

    ImGuiViewport* mainViewport = ImGui::GetMainViewport();
    ImGuiSDL3ViewportData* mainViewportData = (ImGuiSDL3ViewportData*)mainViewport->PlatformUserData;
    ...
    sdlFlags |= useOpengl ? SDL_WINDOW_OPENGL : (bd->useVulkan ? SDL_WINDOW_VULKAN : 0);
    sdlFlags |= SDL_GetWindowFlags(bd->window) & SDL_WINDOW_HIGH_PIXEL_DENSITY;
    sdlFlags |= (viewport->Flags & ImGuiViewportFlags_NoDecoration) ? SDL_WINDOW_BORDERLESS : 0;
    sdlFlags |= (viewport->Flags & ImGuiViewportFlags_NoDecoration) ? 0 : SDL_WINDOW_RESIZABLE;
    sdlFlags |= (viewport->Flags & ImGuiViewportFlags_NoTaskBarIcon) ? SDL_WINDOW_UTILITY : 0;
    sdlFlags |= (viewport->Flags & ImGuiViewportFlags_TopMost) ? SDL_WINDOW_ALWAYS_ON_TOP : 0;
    vd->window = SDL_CreateWindow("No Title Yet", (int)viewport->Size.x, (int)viewport->Size.y, sdlFlags);
...
{
    if (ImGuiSDL3ViewportData* vd = (ImGuiSDL3ViewportData*)viewport->PlatformUserData)
    {
...
{
    ImGuiSDL3ViewportData* vd = (ImGuiSDL3ViewportData*)viewport->PlatformUserData;
...
static void ImGuiSDL3UpdateWindow(ImGuiViewport* viewport)
{
    ...
    SDL_Window* newParent = ImGuiSDL3GetSDLWindowFromViewportID(viewport->ParentViewportId);
...
static int ImGuiSDL3CreateVkSurface(ImGuiViewport* viewport, ImU64 vkInstance, const void* vkAllocator, ImU64* outVkSurface)
{
    ImGuiSDL3ViewportData* vd = (ImGuiSDL3ViewportData*)viewport->PlatformUserData;
    (void)vkAllocator;
    bool ret = SDL_Vulkan_CreateSurface(vd->window, (VkInstance)vkInstance, (VkAllocationCallbacks*)vkAllocator, (VkSurfaceKHR*)outVkSurface);
