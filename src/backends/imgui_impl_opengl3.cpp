...
{
    return ImGui::GetCurrentContext() ? (ImGuiGLData*)ImGui::GetIO().BackendRendererUserData : nullptr;
}
...
static void ImGuiGLSetupRenderState(ImDrawData* drawData, int fbWidth, int fbHeight, GLuint vertexArrayObject)
{
    ImGuiGLData* bd = ImGuiGLGetBackendData();
    ...
    GLenum currentClipOrigin = 0; glGetIntegerv(GL_CLIP_ORIGIN, (GLint*)&currentClipOrigin);
    ...
    if (!clipOriginLowerLeft) { float tmp = t; t = b; b = tmp; } // Swap top and bottom if origin is upper left
    ...
    glUniformMatrix4fv(bd->attribLocationProjMtx, 1, GL_FALSE, &orthoProjection[0][0]);
...
void    ImGuiGLRenderDrawData(ImDrawData* drawData)
{
    ...
    GLuint lastSampler; glGetIntegerv(GL_SAMPLER_BINDING, (GLint*)&lastSampler);
    ...
    for (int n = 0; n < drawData->CmdListsCount; n++)
    {
        ...
        const GLsizeiptr vtxBufferSize = (GLsizeiptr)drawList->VtxBuffer.Size * (int)sizeof(ImDrawVert);
        const GLsizeiptr idxBufferSize = (GLsizeiptr)drawList->IdxBuffer.Size * (int)sizeof(ImDrawIdx);
        ...
        GL_CALL(glBufferSubData(GL_ARRAY_BUFFER, 0, vtxBufferSize, (const GLvoid*)drawList->VtxBuffer.Data));
        GL_CALL(glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, idxBufferSize, (const GLvoid*)drawList->IdxBuffer.Data));
...
bool    ImGuiGLCreateDeviceObjects()
{
    ...
    glGetIntegerv(GL_PIXEL_UNPACK_BUFFER_BINDING, &lastPixelUnpackBuffer); glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
